...
*/
	...
	public String showCustomerEnquiryPage(Model model) {
		...
		CustomerSavingVO customerSavingVO = new CustomerSavingVO();
		model.addAttribute("customerSavingVO", customerSavingVO);
		model.addAttribute("listOptions", customerEnquiryService.findAllAccountType());
		model.addAttribute("locationOptions", customerEnquiryService.findAllLocation());
		return "customer/customerEnquiry"; // customerEnquiry.html
	}
	...
	public String customerAppStatus() {
	  ...
	  public String customerSearch(Model model) {
		  model.addAttribute("listOptions", customerService.allCustomers());
		  return "customer/customerSearch";
	  }
	  
		
		/*
		 * @GetMapping("/customer/customerSearchMain") public String
		 * customerSearchMain(Model model) { model.addAttribute("listOptions",
		 * customerService.allCustomers()); return "customer/customerSearchMain"; }
		 */
	...
	/*
	 * @GetMapping("/customer/addPayeeApproval") public String
	 * customerAddPayeeApproval(Model model) {
	 * 
	 * return "customer/addPayee"; }
	 */
	...
	public String newPayee(@Valid @ModelAttribute("payeeInfoVO") PayeeInfoVO payeeInfoVO,BindingResult bindingResult,HttpSession session, Model model) {
		if(payeeInfoVO.getAccNumberConfirm()!=null && !payeeInfoVO.getAccNumberConfirm().equalsIgnoreCase(payeeInfoVO.getPayeeAccountNo())){
			...
			bindingResult.rejectValue("accNumberConfirm", "account.msg", "An account numbers are not same");
	...
	/*
	 * @GetMapping("/customer/pendingPayee") public String pendinPayeeList(Model
	 * model) { List<PayeeInfoVO> payeeInfoList =
	 * customerService.pendingPayeeList(); model.addAttribute("payeeInfoList",
	 * payeeInfoList); return "customer/pendingPayee";
	 * 
	 * }
	 */
	...
	public String pendinPayeeList(Model model,HttpSession session) {
		LoginVO  loginVO2=(LoginVO)session.getAttribute("userSessionVO");
		String loginid=loginVO2.getUsername();
		List<PayeeInfoVO> payeeInfoList = customerService.pendingPayeeList(loginid);
		model.addAttribute("payeeInfoList", payeeInfoList);
		return "/customer/pendingPayee";	
	}
	/*
	 * @PostMapping("/customer/pendingPayeeRedirect") public String
	 * pendingPayeeRedirect(Model model) { List<PayeeInfoVO> payeeInfoList =
	 * customerService.pendingPayeeList(); model.addAttribute("payeeInfoList",
	 * payeeInfoList); return "customer/pendingPayee";
	 * 
	 * }
	...
	public String registeredPayeeList(Model model,HttpSession session) {
		LoginVO  loginVO2=(LoginVO)session.getAttribute("userSessionVO");
		String loginid=loginVO2.getUsername();
		List<PayeeInfoVO> payeeInfoList = customerService.registeredPayeeList(loginid);
		model.addAttribute("payeeInfoList", payeeInfoList);
	...
	@PostMapping("/customer/fundTransferConfirm")
	public String fundTransferConfirm(@Valid @ModelAttribute("fundTransferVO") FundTransferVO fundTransferVO, BindingResult bindingResult, Model model) {
		if (bindingResult.hasErrors()) {
		 	return "customer/fundTransfer";	
		}else {	
		int n = PasswordGenerator.getOPT();
		fundTransferVO.setOtp(n);;
		System.out.println(fundTransferVO);
		System.out.println("Otp = "+fundTransferVO.getOtp());
		//customerService.addFundTransfer(fundTransferVO);
		model.addAttribute("fundTransferVO",fundTransferVO);
		return "customer/fundTransferConfirm";	
		}
	}
	
	@PostMapping("/customer/fundTransferReview")
	public String fundTransferReview(@ModelAttribute("fundTransferVO") FundTransferVO fundTransferVO,Model model) {
		model.addAttribute("fundTransferVO",fundTransferVO);
		return "customer/fundTransferred";	
	}
	
	
	@GetMapping("/load/image")
	public void findPhotoByEmail(@RequestParam String email, HttpServletResponse response) throws IOException{
		response.setContentType("image/png");
		
		byte[] photo=customerService.findPhotoByEmail(email);
	...
	@PostMapping("/customer/cardGenerator")
	public String generateCard(@RequestParam("customerEmail") String customerEmail,@RequestParam("customerName") String customerName){ 
		customerService.creatCreditCard(customerName, customerEmail);
		return "redirect:/customer/customerSearch";
	}
	...
	/*
	 * @GetMapping("/customers/credit/card") public void
	 * findCustomerCreditCardfront(@RequestParam String name,@RequestParam String
	 * email,HttpServletResponse response) throws IOException { byte[]
	 * card=customerService.generatedCreditCard(name, email);
	 * response.setContentType("image/png"); ServletOutputStream
	 * outputStream=response.getOutputStream(); if(card!=null) {
	 * outputStream.write(card); }else { outputStream.write(new byte[] {}); }
	 * outputStream.flush(); outputStream.close(); }
	 * 
	 * 
	 * @GetMapping("/customers/credit/cardback") public void
	 * findCustomerCreditCardback(@RequestParam String name,@RequestParam String
	 * email,HttpServletResponse response) throws IOException { byte[]
	 * card=customerService.generateBackCreditCard(name,email);
	 * response.setContentType("image/png"); ServletOutputStream
	 * outputStream=response.getOutputStream(); if(card!=null) {
	 * outputStream.write(card); }else { outputStream.write(new byte[] {}); }
	 * outputStream.flush(); outputStream.close(); }
